...
function initGridOverlay(element) {
  ...
  element.addEventListener("mousemove", (e) => {
    ...
    if (closestBlock.shouldScramble && !closestBlock.sccrambleInterval) {
      closestBlock.sccrambleInterval = setInterval(() => {
        closestBlock.element.textContent = getRandomSymbol();
      }, config.sccrambleInterval);
    }
    ...
    for (let i = 0; i < clusterCount; i++) {
      ...
      randomNeighbour.highlightEndTime =
        currentTime + config.blockLifetime + i * 10;

      if (
        randomNeighbour.shouldScramble &&
        !randomNeighbour.sccrambleInterval
      ) {
        randomNeighbour.sccrambleInterval = setInterval(() => {
  ...
  function updateHighlights() {
    ...
    blocks.forEach((block) => {
      if (block.highlightEndTime > 0 && currentTime > block.highlightEndTime) {
        ...
        if (block.sccrambleInterval) {
          clearInterval(block.sccrambleInterval);
          block.sccrambleInterval = null;
          if (!block.isEmpty) {
